...
export default async function TopicsList({
  ...
  const { topics, totalPages } = await getTopicsPerCategory(categoryId, page);
  return (
    <div>
      <PaginationLinks currentPage={page} numberOfPages={totalPages} />
      <Card key={categoryId}>
        <CardHeader canMinimise>{categoryName}</CardHeader>
        <CardContent noPadding>
          <table className="w-full table-fixed">
            ...
            <tbody>
              {topics.map((topic) => {
                console.log("latestpost", topic.latestPost);
                return (
                  <tr className="border-solid border-b-2 border-slate-200 last:border-b-0">
                    ...
                    <td className="hidden lg:table-cell p-2 text-center bg-slate-50">
                      <LatestPost
                        ...
                        hideTitle={true}
                        title={topic.title}
                        username={topic.latestPost.user.username}
                      />
                    </td>
                  </tr>
                );
